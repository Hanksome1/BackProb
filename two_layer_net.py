...
def sample_batch(
    ...
    torch.manual_seed(0)
    indices = torch.randint(num_train, (batch_size,)) # gerenate random indices
    X_batch = X[indices] # get X_batch
    y_batch = y[indices] # get y_batch
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.maximum(hidden, torch.zeros_like(hidden)) # ReLU
